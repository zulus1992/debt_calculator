...
--
-- Бот работает через REST (PostgREST) с secret-ключом базы (sb_secret_…) или legacy
-- service_role, поэтому RLS включён и политик нет: публичные ключи (publishable/anon)
-- и роли anon/authenticated доступа не получат — читать и писать может только бот.
