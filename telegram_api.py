...
class TelegramBot:
    ...
    def set_my_commands(self, commands: Sequence[tuple[str, str]]) -> bool:
        ...
        подсвечивает в сообщениях бота как нажимаемые: по совету «нажмите /reg» или
        «/login» можно тапнуть — команда встанет в строку ввода, набирать её руками
        не нужно. Поэтому команда, которую бот советует текстом, должна быть в списке.
