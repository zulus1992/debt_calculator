...
4. Локально (там, где лежит проект с заполненным `.env`) сказать Telegram, куда присылать апдейты:
   ...
   python bot.py --check                            # заодно покажет, что Telegram знает о командах
   ```
   `--set-webhook` объявляет в Telegram список команд сам: меню «/» и нажимаемые `/reg` и `/login`
   берутся только из него, а в режиме вебхука `python bot.py` (который объявляет список при
   запуске) не работает. Если список команд менялся в коде, обновите его вручную:
   `python bot.py --set-commands`.
...
| В Telegram нет меню «/», команды в ответах бота не нажимаются | список команд не объявлен: в режиме вебхука `python bot.py` не запускается, и объявить его некому. Выполните `python bot.py --set-commands`, а посмотреть, что Telegram знает сейчас, — `python bot.py --check` (подробнее — в таблице ниже) |
...
```powershell
python -m unittest tests.test_pipeline   # 385 тестов, без внешних сервисов
python bot.py --check                    # проверка ключей, сервисов и списка команд в Telegram
```
...
| В Telegram нет меню «/» и команды в ответах бота не нажимаются | список команд не был объявлен: в режиме вебхука `python bot.py` не запускается, и объявить его некому. Смотрите `python bot.py --check` — он печатает, сколько команд знает Telegram, — и выполните `python bot.py --set-commands` (или заново `--set-webhook`: он объявляет список сам). В личном чате с ботом меню — кнопка у поля ввода, в группе список появляется, когда наберёте «/». Сами ответы бота при этом работают: без списка теряются только подсказки и подсветка команд |
...
хостинга: она печатает статус Telegram, DeepSeek, Supabase, таблицы курсов, признак пароля
и объявленный в Telegram список команд.
