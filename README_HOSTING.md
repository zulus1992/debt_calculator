...
| `permission denied for schema public` (42501) | у роли, которую выбрал шлюз, нет прав на схему: либо ключ не secret (нужен `sb_secret_…`, а не publishable/anon), либо шлюз ждёт ключ ещё и в `Authorization`. Запустите `python bot.py --check`: он покажет тип ключа и какой из двух режимов заголовков принимает база. Если работают только «apikey + Authorization» — поставьте `SUPABASE_KEY_HEADER=both` в `.env` (на хостинге — в переменных окружения) |
